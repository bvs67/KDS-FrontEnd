...
export class StrucTree extends React.Component {
    ...
    _getClassName(node) {
        return (
            (node === this.props.CurrentNode ? "info2" : "info")
        )
    }
...
//    }
    ...
    render2 = () => {
        ...
        const renderTree = nodes => (
            <TreeView key={nodes.id} nodeLabel={<a href="#" className={this._getClassName(nodes.id)} 
                                      onClick={this._onClick.bind(this, nodes.id)}>{nodes.Name}</a>} defaultCollapsed={false}>
